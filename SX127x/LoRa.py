...
class LoRa(object):
    ...
    def __init__(self, verbose=True, do_calibration=True, calibration_freq=868):
        ...
        BOARD.add_events(
            self._dio0, self._dio1, self._dio2, self._dio3, self._dio4, self._dio5
        )
    ...
    def rx_is_good(self):
        ...
        return not any(
            [flags[s] for s in ["valid_header", "crc_error", "rx_done", "rx_timeout"]]
        )
    ...
    def set_freq(self, f):
        ...
        assert (
            ...
            or self.mode == MODE.STDBY
            or self.mode == MODE.FSK_STDBY
        )
        i = int(f * 16384.0)  # choose floor
        msb = i // 65536
        i -= msb * 65536
        mid = i // 256
        i -= mid * 256
    ...
    def get_pa_config(self, convert_dBm=False):
        ...
        if convert_dBm:
            max_power = max_power * 0.6 + 10.8
            if pa_select == 0:
                output_power = max_power - (15 - output_power)
            else:
                output_power = 17 - (15 - output_power)
        return dict(pa_select=pa_select, max_power=max_power, output_power=output_power)
    ...
    def get_ocp(self, convert_mA=False):
        ...
        if convert_mA:
            if ocp_trim <= 15:
                ocp_trim = 45.0 + 5.0 * ocp_trim
            elif ocp_trim <= 27:
                ocp_trim = -30.0 + 10.0 * ocp_trim
            else:
                assert ocp_trim <= 27
        return dict(ocp_on=ocp_on, ocp_trim=ocp_trim)

    def set_ocp_trim(self, I_mA):
        assert I_mA >= 45 and I_mA <= 240
        ocp_on = self.spi.xfer([REG.LORA.OCP, 0])[1] >> 5 & 0x01
        if I_mA <= 120:
            v = int(round((I_mA - 45.0) / 5.0))
        else:
            v = int(round((I_mA + 30.0) / 10.0))
        v = set_bit(v, 5, ocp_on)
        return self.spi.xfer([REG.LORA.OCP | 0x80, v])[1]

    def get_lna(self):
        v = self.spi.xfer([REG.LORA.LNA, 0])[1]
        return dict(lna_gain=v >> 5, lna_boost_lf=v >> 3 & 0b11, lna_boost_hf=v & 0b11)
    ...
    def set_lna(self, lna_gain=None, lna_boost_lf=None, lna_boost_hf=None):
        ...
        loc = {s: current[s] if loc[s] is None else loc[s] for s in loc}
        val = (
            (loc["lna_gain"] << 5) | (loc["lna_boost_lf"] << 3) | (loc["lna_boost_hf"])
        )
    ...
    def get_pkt_snr_value(self):
        ...
        return (float(v - 256) if v > 127 else float(v)) / 4.0
    ...
    def get_modem_config_3(self):
        ...
        return dict(low_data_rate_optim=val >> 3 & 0x01, agc_auto_on=val >> 2 & 0x01)
    ...
    def get_symb_timeout(self):
        ...
        msb, lsb = self.spi.xfer([SYMB_TIMEOUT_MSB, 0, 0])[
            1:
        ]  # the MSB bits are stored in REG.LORA.MODEM_CONFIG_2
        msb = msb & 0b11
        return lsb + 256 * msb
    ...
    def set_symb_timeout(self, timeout):
        ...
        reg_modem_config_2 = (
            bkup_reg_modem_config_2 & 0xFC | msb
        )  # bits 2-7 of bkup_reg_modem_config_2 ORed with the two msb bits
        old_msb = (
            self.spi.xfer([REG.LORA.MODEM_CONFIG_2 | 0x80, reg_modem_config_2])[1]
            & 0x03
        )
    ...
    def get_dio_mapping_2(self, mapping):
        ...
        :rtype: list[int]
        """
        self.dio_mapping = self.dio_mapping[0:4] + [
            mapping >> 6 & 0x03,
            mapping >> 4 & 0x03,
        ]
        return self.dio_mapping
    ...
    def rx_chain_calibration(self, freq=868.0):
